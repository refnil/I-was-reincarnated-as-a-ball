//! An example game written in the Bevy game engine and using the [`agb`] crate to allow running it
//! on the GameBoy Advance.

//! We declare our crate as `no_std`, as the GameBoy Advance doesn't have a port of the standard
//! library.
#![no_std]

//! We also declare the crate as not having a typical `main` function.
//! The `agb-gbafix` tool we use to generate our final ROM file expects an exported
//! function named `main` accepting no arguments and _never_ returning.
//! This is handled by [`main`].
#![no_main]

//! [`agb`] provides a global allocator, allowing us to use items from the [`alloc`] crate.

extern crate alloc;

use bevy::prelude::*;
use lib::{
    ball_type::{BallPlugin, EnemyBall, PlayerBall},
    base::BasePlugin,
    physic::{PhysicConfig, PhysicObject, PhysicPlugin},
    render::{AffineSprite, RepeatedSprite},
    sprite_loader::Sprites,
};

/// Main entry point.
#[expect(unsafe_code)]
#[unsafe(export_name = "main")]
pub extern "C" fn main() -> ! {
    // We can use Bevy's `App` abstraction just like any other Bevy application.
    let mut app = App::new();

    app.add_plugins(BasePlugin);
    app.add_systems(Startup, setup_balls);
    app.insert_resource(PhysicConfig::screen_boundary());
    app.add_systems(Update, do_rotate);
    app.add_systems(Update, do_scale);

    app.run();

    // Finally, we ensure this function never returns by entering an infinite loop if our app
    // ever exits.
    #[allow(clippy::empty_loop)]
    loop {}
}

#[derive(Component)]
struct Rotate;

#[derive(Component)]
struct Scale;

fn setup_balls(mut commands: Commands, sprites: NonSend<Sprites>) {
    commands.spawn((
        Transform::from_xyz(20., 20., 0.),
        Rotate,
        Scale,
        PlayerBall::Boy.to_bundle(&sprites, true),
    ));

    commands.spawn((
        Transform::from_xyz(100., 20., 0.),
        Rotate,
        PlayerBall::Boy.to_bundle(&sprites, true),
    ));

    commands.spawn((
        Transform::from_xyz(20., 100., 0.),
        Scale,
        PlayerBall::Boy.to_bundle(&sprites, true),
    ));
}

fn do_rotate(balls: Query<&mut Transform, With<Rotate>>) {
    for mut transform in balls {
        transform.rotate_z(0.1);
    }
}

fn do_scale(balls: Query<&mut Transform, With<Scale>>) {
    for mut transform in balls {
        transform.scale += Vec3::new(0.1, 0.1, 0.);

        if transform.scale.x > 2. {
            transform.with_scale(Vec3::ONE);
        }
    }
}
